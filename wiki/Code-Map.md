...
| File | Purpose |
|---|---|
| `app/models/node_event.rb` | Persisted node events (12 types including trunk/satellite) with scopes for analytics |
...
| `app/models/tg.rb` | Talkgroups with `kind` (local/remote/cluster) for GeuReflector classification |
...
| `app/controllers/admin/reflector_controller.rb` | Web admin for GeuReflector/SVXReflector configuration (reads/writes config incl. trunks/satellites, restarts container) |
...
| Directory | Purpose |
|---|---|
| `app/views/dashboard/` | Dashboard index (with trunk/satellite panels), map (with trunk info), stats (with network stats), TG matrix |
...
| `app/views/admin/reflector/` | GeuReflector/SVXReflector config editor (global, clustering/trunking, certs, users, passwords, TG rules) |
...
| File | Purpose |
|---|---|
| `lib/reflector_listener.rb` | Polls reflector status API, diffs node/trunk/satellite/cluster state, broadcasts updates, persists events, fetches /config, enriches nodes with RX metadata |
| `app/services/reflector_config.rb` | Parses/generates `svxreflector.conf` including `[TRUNK_x]` and `[SATELLITE]` sections |
...
| `docker-compose.yml` | Service definitions: svxreflector (ports 5300/5302/5303), caddy, web, updater, audio_bridge, redis |
...
- **`node_events`** — indexed on `callsign`, `event_type`, `created_at`, `tg`, `source` (trunk attribution)
- **`users`** — callsign, password digest, role, permissions
- **`tgs`** — talkgroup definitions with `kind` (local/remote/cluster)
- **`bridges`** — bridge instances (80+ columns for all protocol types)
- **`bridge_tg_mappings`** — talkgroup mappings for bridges
