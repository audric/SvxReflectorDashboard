...
// C wrapper for DroidStar MBEVocoder
extern void* mbe_vocoder_new();
extern void  mbe_vocoder_free(void* voc);
extern void  mbe_vocoder_decode_dstar(void* voc, uint8_t* ambe, int16_t* pcm);
extern void  mbe_vocoder_encode_dstar(void* voc, int16_t* pcm, uint8_t* ambe);
*/
import "C"
...
const (
	...
	AMBEFrameSize = 9    // 9 bytes = 72 bits D-STAR AMBE
)

// Vocoder wraps DroidStar's MBEVocoder for D-STAR AMBE 2400x1200.
...
// Decode converts a 9-byte D-STAR AMBE frame to 160 PCM samples (8kHz, 16-bit mono).
func (v *Vocoder) Decode(ambe [9]byte) [PCMFrameSize]int16 {
	v.mu.Lock()
	defer v.mu.Unlock()

	var pcm [PCMFrameSize]int16
	C.mbe_vocoder_decode_dstar(
		v.handle,
...
// Encode converts 160 PCM samples (8kHz, 16-bit mono) to a 9-byte D-STAR AMBE frame.
func (v *Vocoder) Encode(pcm [PCMFrameSize]int16) [9]byte {
	v.mu.Lock()
	defer v.mu.Unlock()

	var ambe [9]byte
	C.mbe_vocoder_encode_dstar(
		v.handle,
		(*C.int16_t)(unsafe.Pointer(&pcm[0])),
		(*C.uint8_t)(unsafe.Pointer(&ambe[0])),
	)
